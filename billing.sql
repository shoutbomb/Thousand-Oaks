...
WBExport -type=text
                 -file='c:/Shoutbomb/FTP/Billing/billing$[today].txt'
                 -delimiter='|'
                 -quotechar='"'
				 ...
				 SELECT 
				   ...
				   round(p.owed_amt,2)                     AS total_money_owed,
				 ...
				 FROM sierra_view.checkout AS c
				   ...
				   LEFT JOIN sierra_view.fine as f
				   ON f.patron_record_id = p.record_id
				 WHERE
				   --(current_date - c.due_gmt::date) > 30 
				   i.item_status_code = 'n' AND p.owed_amt > '0'
				   AND f.assessed_gmt >= '2019=10-01'
